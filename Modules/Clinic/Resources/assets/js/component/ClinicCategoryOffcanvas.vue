          ...
          <div class="col-md-6">
            ...
            <div class="image-upload-container text-center">
              ...
              <div class="avatar-preview" style="width: 120px; height: 120px; border-radius: 50%; overflow: hidden; margin: 0 auto;">
                <img :src="image_url || '/path/to/default-avatar.jpg'" alt="Profile Image" style="width: 100%; height: 100%; object-fit: cover;" />
              </div>
              ...
              <button type="button" class="btn btn-danger mt-2 ml-3" :disabled="!image_url" @click="removeImage">{{ $t('messages.remove') }}</button>
            </div>
...
const triggerFileInput = () => {
  ...
  fileInput.type = 'file';
  fileInput.accept = '.jpeg, .jpg, .png, .gif';  // Define accepted file types

  // When the user selects a file, trigger the change event to handle the file
...
const onImageChange = (event) => {
  ...
  if (file) {
    ...
    reader.onload = () => {
      // Set the image as base64 string for preview once the file is loaded
      image_url.value = reader.result;  // This updates the image_url ref
    }
...
const removeImage = () => {
  image_url.value = null;  // Clear the image preview
}
